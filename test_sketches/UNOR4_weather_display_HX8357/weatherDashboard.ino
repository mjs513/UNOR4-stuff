...
void drawWeatherDashboard() {
  ...
  //step();
  drawKeyboard(getWeatherIconFilename(100), 420, 10);
  ...
  drawPNG(image, 175, 80);
  drawAQIMetric(45, 150, weather.AQI);

  // Current Temperature Data
  SetTFTFont(&FreeSans12pt7b);
  tft.setTextColor(COLOR_YELLOW);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    tft.setTextColor(COLOR_ORANGE);
    tft.setCursor(x+5, 290);
    tft_printf("%.1fF", weather.tempsHigh[i]);

    tft.setTextColor(COLOR_CYAN);
    tft.setCursor(x+5, 305);
