...
void drawPNG(const char *filename, int x, int y) {
    ...
    if (image_info->palette == nullptr) {
        tft.writeRect(x, y + image_info->image_top, 64, (image_bottom - image_info->image_top) + 1, (const uint16_t *)image_info->image);
    } else {
        tft.writeRect8BPP(x, y + image_info->image_top, 64, (image_bottom - image_info->image_top) + 1, (const uint8_t *)image_info->image, image_info->palette);
